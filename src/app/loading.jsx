		...
		<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col items-center">
			...
			<div className="flex flex-wrap justify-center gap-6 mb-16 w-full">
				{[1, 2, 3, 4].map((i) => (
					<div
						key={i}
						className="w-40 h-24 bg-gray-100 rounded-xl animate-pulse shadow-sm flex flex-col items-center justify-center relative overflow-hidden">
						<div className="absolute inset-0 -translate-x-full bg-linear-to-r from-transparent via-white/60 to-transparent animate-shimmer"></div>
					</div>
				))}
			</div>
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
				{[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
					<div
						key={i}
						className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center animate-pulse relative overflow-hidden">
